...
final class UpdateChecker: ObservableObject {
    ...
    private static let manifestName = "LocalMonitor.dmg.update.json"
    private static let productionBundleIdentifier = "dev.local.LocalMonitor"
    private static let appBundleName = "Local Monitor.app"
    private static let executableName = "LocalMonitor"
        ...
        let task = URLSession.shared.downloadTask(with: downloadURL) { [weak self] tmpURL, response, error in
            Task { @MainActor in
                ...
                let workDirectory = FileManager.default.temporaryDirectory
                    .appendingPathComponent("localmonitor-update-\(UUID().uuidString)", isDirectory: true)
