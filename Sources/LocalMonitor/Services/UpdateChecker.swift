...
final class UpdateChecker: ObservableObject {
    ...
    private static let repo = "localmonitor"
    private static let assetName = "LocalMonitor.dmg"
    private static let productionBundleIdentifier = "dev.local.LocalMonitor"
    ...
    func checkForUpdates(force: Bool = false) async {
        ...
        request.setValue("LocalMonitor-UpdateChecker", forHTTPHeaderField: "User-Agent")
    ...
    func downloadAndInstall() {
        ...
        let task = URLSession.shared.downloadTask(with: downloadURL) { [weak self] tmpURL, _, error in
            Task { @MainActor in
                ...
                let fileName = "LocalMonitor-\(self.latestVersion ?? "latest")-\(UUID().uuidString).dmg"
    ...
    private func installUpdate(dmgURL: URL, expectedVersion: String) {
        ...
        let targetBundle = currentBundle.path.hasPrefix("/Applications/")
            ? currentBundle
            : URL(fileURLWithPath: "/Applications/Local Monitor.app")

        let scriptURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("local-monitor-install-\(UUID().uuidString).sh")
        let logURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("local-monitor-install.log")
        let mountPointURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("local-monitor-mount-\(UUID().uuidString)")
        ...
        STAGED_APP="$TARGET_PARENT/.Local Monitor.app.update.$$"
        BACKUP_APP="$TARGET_PARENT/.Local Monitor.app.previous.$$"
        MOUNT_ATTACHED=0
        ...
        SRC="$MOUNT_POINT/Local Monitor.app"
        if [ ! -d "$SRC" ]; then
            fail "source app not found at $SRC"
        fi

        SRC_INFO="$SRC/Contents/Info.plist"
        SRC_EXEC="$SRC/Contents/MacOS/LocalMonitor"
