...
APP_PATH="${1:-$ROOT_DIR/.build/Local Monitor.app}"
APP_BUNDLE_NAME="$(basename "$APP_PATH")"
APP_DISPLAY_NAME="${APP_BUNDLE_NAME%.app}"
OUTPUT_DMG="${2:-$ROOT_DIR/LocalMonitor.dmg}"

VOLUME_NAME="${VOLUME_NAME:-Local Monitor}"
DMG_TITLE="${DMG_TITLE:-Local Monitor, built for}"
DMG_SUBTITLE="${DMG_SUBTITLE:-local dev visibility}"
DMG_HINT="${DMG_HINT:-Drag Local Monitor to Applications}"
...
arrow.lineWidth = 2
arrow.move(to: NSPoint(x: width / 2 - 44, y: CGFloat(296)))
arrow.line(to: NSPoint(x: width / 2 + 44, y: CGFloat(296)))
arrow.move(to: NSPoint(x: width / 2 + 30, y: CGFloat(310)))
arrow.line(to: NSPoint(x: width / 2 + 44, y: CGFloat(296)))
arrow.line(to: NSPoint(x: width / 2 + 30, y: CGFloat(282)))
color(142, 152, 146).setStroke()
arrow.stroke()
